...
def test_prepare_speculator_attn_rebuilds_global_kv_layout() -> None:
    ...
    manager._gathered_kv_slot_mappings = torch.empty(
        (1, 40), dtype=slot_mappings.dtype
    )
    manager._global_batch = global_batch

    result_block_tables, result_slot_mappings = (
        manager.prepare_speculator_attn(global_batch)
    )
...
def test_ascend_pcp_manager_is_registered() -> None:
    manager_cls = PCPManagerRegistry.get_manager_class(
        ASCEND_PCP_MANAGER_NAME
    )
    assert manager_cls is AscendPCPManager
