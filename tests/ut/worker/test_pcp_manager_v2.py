...
def test_partition_batch_pads_decode_to_fixed_full_graph_shape(num_reqs):
    ...
    expected_attn_state = (
        pcp_manager_module.AscendAttentionState.DecodeOnly if source_attn_state is None else source_attn_state
    )
    ...
    expected_query_start_loc = np.arange(
        graph_num_reqs + 1,
        dtype=np.int32,
    )
    ...
    expected_input_ids = [
        *range(100, 100 + num_reqs),
        *([0] * (graph_num_reqs - num_reqs)),
    ]
    expected_positions = [
        *range(10, 10 + num_reqs),
        *([0] * (graph_num_reqs - num_reqs)),
    ]
    expected_padding = [False] * num_reqs + [True] * (graph_num_reqs - num_reqs)
