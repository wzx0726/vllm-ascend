...
def test_speculator_rebuilds_global_pcp_attention(
    ...
    global_aux_hidden_states = torch.arange(
        14, dtype=torch.float32
    ).reshape(2, 7)
    aux_hidden_states = (
        local_aux_hidden_states if method == "eagle3" else None
    )
    ...
    speculator.pcp_manager.restore_hidden_states.return_value = (
    ...
    if method == "eagle3":
        restore_args = (
            speculator.pcp_manager.restore_hidden_states.call_args.args
        )
