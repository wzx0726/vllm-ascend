...
def test_prepare_replicated_pcp_attn_uses_speculator_owned_global_inputs() -> None:
    speculator = object.__new__(AscendMTPSpeculator)
    speculator.pcp_manager = MagicMock()
    ...
    input_batch.query_start_loc = torch.tensor([0, 3, 6], dtype=torch.int32)
    input_batch.positions = torch.arange(6, dtype=torch.int64)

    block_tables = (MagicMock(),)
    global_slot_mapping = torch.arange(6).unsqueeze(0)
    ...
    speculator.block_tables.compute_slot_mappings.assert_called_once_with(
        ...
        num_tokens_padded=input_batch.num_tokens,
    )
...
def test_speculator_uses_replicated_pcp_attention(
    ...
    """Verify global eager proposal inputs for both MTP and Eagle3."""
    ...
    prepare_replicated_pcp_attn.assert_called_once_with(eager_batch)

    propose_args = propose.call_args.args
    assert propose_args[0] is eager_batch
