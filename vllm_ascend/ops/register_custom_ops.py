...
# ---------------------------------------------------------------------------
# npu_allto_all_attn_update_all_gather — vllm:: namespace variant
#
# The operator is also registered as torch.ops._C_ascend.* (via
# csrc/torch_binding.cpp + TORCH_LIBRARY_EXPAND) for the npugraph_ex path.
# This vllm:: variant is insurance for the inductor / fusion_pass path
# (PIECEWISE / NONE cudagraph_mode, where enable_npugraph_ex is forced False
# by platform.py).
#
# Functionalization: direct_register_custom_op + mutates_args auto-generates
# the Functionalize dispatch (verified empirically — upstream vllm has ~30
# inplace ops registered this way with zero hand-written Functionalize, e.g.
# flashinfer_rotary_embedding, and they all run inside the inductor-compiled
# graph). So no hand-written _functional block is needed; relying on automatic
# functionalization per the design decision NOT to add this op to
# splitting_ops. (mla_forward uses the same pattern but is NOT a valid witness
# here — it is forced eager via platform.py splitting_ops, so it never goes
# through functionalization.)
#
# op_func wraps the _C_ascend implementation (inplace on attn and lse). Both
# _impl and _fake return None: the mutated tensors are bound via mutates_args,
# not via a return value. The _C_ascend op is registered lazily; resolution of
# torch.ops._C_ascend.npu_* happens at call time, not registration time.
# ---------------------------------------------------------------------------
def _npu_allto_all_attn_update_all_gather_impl(
    attn: torch.Tensor,
    lse: torch.Tensor,
...
def _npu_allto_all_attn_update_all_gather_fake(
    attn: torch.Tensor,
    lse: torch.Tensor,
    mask_num: torch.Tensor,
    group: str,
...
direct_register_custom_op(
    op_name="npu_allto_all_attn_update_all_gather",
    op_func=_npu_allto_all_attn_update_all_gather_impl,
    fake_impl=_npu_allto_all_attn_update_all_gather_fake,
